...
class TestRunPipeline:
    ...
    def test_step_order(
        ...
        # check-quality runs twice: once on input, once in report step
        assert call_order == [
            "archive",
            "check-quality",
            "process",
...
class TestCoalesceSkip:
    ...
    def test_coalesce_runs_when_splits(
        ...
        # First call (input check) reports splits; second (report) clean
        mock_quality.side_effect = [
            QualityReport(split_escapes=5, split_sync_updates=0),
            QualityReport(split_escapes=0, split_sync_updates=0),
        ]
...
class TestStepExceptionHandling:
    ...
    def test_run_check_quality_runtime_error_returns_fail(self) -> None:
        ...
        with patch(f"{_PATCH_BASE}.check_quality", side_effect=RuntimeError("bad")):
            step, has_splits = _run_check_quality("/tmp/in.clirec")
